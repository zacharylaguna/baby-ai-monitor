...
def lanesDetection(img):
    ...
    lines = cv.HoughLinesP(cropped_image, rho=2, theta=np.pi/180,
                           threshold=150, lines=np.array([]), minLineLength=10, maxLineGap=30)
    # lines = cv.HoughLines(cropped_image, rho=2, theta=np.pi/180,
    #                        threshold=50, lines=np.array([]))
    # lines = cv.HoughLines(cropped_image, 1, np.pi / 180, 150, None, 0, 0)
